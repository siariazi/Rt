...
ref_strain
    ...
    group <- Group$new(
        ...
        rec_shape = 14, rec_rate = 2. # gamma distn for recovery shape 
    )
group

    index_cases <- group$infectious_hosts(0)
    index_cases
    index_rec_times <- vapply(index_cases, function(h) h$recovery_time, numeric(1L)) # finding the index of 
    index_rec_times
